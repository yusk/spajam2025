...
class UserSignUpSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs["password"] != attrs["password_confirm"]:
            raise ValidationError({"password_confirm": "パスワード確認がパスワードと一致していません。"})
...
class UserPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if attrs["new_password"] != attrs["new_password_confirm"]:
            raise ValidationError({"new_password_confirm": "パスワード確認がパスワードと一致していません。"})
        if attrs["password"] == attrs["new_password"]:
            raise ValidationError({"new_password": "新しいパスワードは現在のパスワードと異なってなければなりません"})
        return attrs
